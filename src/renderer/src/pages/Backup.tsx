    ...
    <>
      <div className="h-full max-w-full space-y-6 ">
        ...
        {loading ? (
          <div className="flex items-center justify-center h-96">
            <Loader2 size={32} className="text-zevyron-primary animate-spin" />
          </div>
        ) : filteredRestorePoints.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center bg-zevyron-card border border-zevyron-border rounded-lg">
            <div className="p-4 bg-zevyron-secondary rounded-full mb-4">
              <Shield size={28} className="text-zevyron-text" />
            </div>
            <h3 className="text-lg font-medium mb-2 text-zevyron-text">No Restore Points Found</h3>
            <p className="text-zevyron-text-secondary max-w-sm mb-4">
              {searchQuery
        ...
        <p className="text-center text-zevyron-text-muted mt-4">
      ...
      >
        {modalState.type === "restore" && modalState.restorePoint && (
          <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-4 shadow-xl max-w-lg w-full mx-4 pb-0">
            <h3 className="text-lg font-medium text-zevyron-text">Restore System</h3>
            ...
            <div className="p-4 pr-0">
              <p className="text-zevyron-text-secondary mb-4">
                ...
                may be lost.
                <br /> <br />
                This will revert all changes Zevyron has made to your system since this restore
                point was created.
              </p>
      ...
      <Modal open={customModalOpen} onClose={() => !processing && setCustomModalOpen(false)}>
        <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-4 shadow-xl max-w-lg w-full mx-4 pb-0">
          <h3 className="text-lg font-medium text-zevyron-text">Create Custom Restore Point</h3>
          ...
          <div className="p-4 space-y-4">
            ...
            <p className="text-xs text-center text-zevyron-text-muted">
      ...
      <Modal open={confirmDeleteAll} onClose={() => !processing && setConfirmDeleteAll(false)}>
        <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-4 shadow-xl max-w-lg w-full mx-4 pb-0">
          <h3 className="text-lg font-medium text-zevyron-text">Delete All Restore Points</h3>
          <div className="p-4 pr-0">
            <p className="text-zevyron-text-secondary mb-4">
  ...
  return (
    <>
      <div className="h-full max-w-full space-y-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <p className="text-zevyron-text-secondary text-sm">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center bg-zevyron-card border border-zevyron-border rounded-lg">
            <div className="p-4 bg-zevyron-secondary rounded-full mb-4">
              <Wrench size={28} className="text-zevyron-text" />
            </div>
            <h3 className="text-lg font-medium mb-2 text-zevyron-text">No Applied Tweaks</h3>
            <p className="text-zevyron-text-secondary max-w-sm mb-4">
              ...
              {appliedTweaks.map((tweak) => (
                <TableRow key={tweak.name}>
                  <TableCell>
                    ...
                    {tweak.description && (
                      <p className="text-xs text-zevyron-text-secondary mt-0.5">
                        {tweak.description}
                      </p>
                    )}
                  </TableCell>
                  <TableCell className="text-center">
                    {tweak.reversible !== false ? (
                      ...
                      <span className="text-zevyron-text-secondary text-xs">Not reversible</span>
                    )}
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Modal open={confirmUndoAll} onClose={() => !processing && setConfirmUndoAll(false)}>
        <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-4 shadow-xl max-w-lg w-full mx-4 pb-0">
          <h3 className="text-lg font-medium text-zevyron-text">Undo All Tweaks</h3>
          <div className="p-4 pr-0">
            <p className="text-zevyron-text-secondary mb-4">
      ...
      <Modal open={showWhyNotReversible} onClose={() => setShowWhyNotReversible(false)}>
        <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-4 shadow-xl max-w-lg w-full mx-4">
          <h3 className="text-lg font-medium text-zevyron-text mb-4">
            Why are some tweaks not reversible?
          </h3>
          <div className="text-zevyron-text-secondary text-sm leading-6 space-y-3">
            <p>
              Some tweaks make changes that can't be automatically reversed by Zevyron. However,
              most of these changes can still be undone manually:
            </p>
            <ul className="list-disc list-inside space-y-2 ml-2">
              <li>
                <strong className="text-zevyron-text">Debloating Windows:</strong> Removed apps can
                be reinstalled from the Microsoft Store
              </li>
              <li>
                <strong className="text-zevyron-text">Optimize NVIDIA Settings:</strong> Settings
                can be reset through the NVIDIA Control Panel
              </li>
              <li>
                <strong className="text-zevyron-text">Service modifications:</strong> Services can
  ...
  return (
    <RootDiv>
      <div className="h-full max-w-full space-y-4 overflow-hidden">
        <div className="flex gap-1 bg-zevyron-card border border-zevyron-border rounded-lg p-1 w-fit">
          <button
            onClick={() => setActiveTab("restore")}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all active:scale-95 ${
              activeTab === "restore"
                ? "bg-zevyron-primary text-white shadow"
                : "text-zevyron-text-secondary hover:text-zevyron-text"
            }`}
          >
