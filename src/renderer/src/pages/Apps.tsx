...
import logo from "../../../../resources/zevyron-icon.png"
...
const APPS_CACHE_KEY = "zevyron.apps.json"
  ...
  return (
    ...
    >
      {visible ? (
        ...
        <Card className="p-4 animate-pulse border-zevyron-border">
          <div className="flex items-center gap-4">
            <div className="w-8 h-8 rounded-lg bg-zevyron-accent" />
            <div className="space-y-2 flex-1">
              <div className="h-3 w-1/2 rounded bg-zevyron-accent" />
              <div className="h-2 w-3/4 rounded bg-zevyron-accent/60" />
            </div>
          </div>
        </Card>
      )}
...
function Apps() {
  ...
  const installChocolatey = async (): Promise<void> => {
    ...
    try {
      ...
      toast.success("Chocolatey installed! Please restart Zevyron to continue.", {
        autoClose: false,
      })
    ...
    <>
      <Modal open={showWelcomeModal} onClose={() => setShowWelcomeModal(false)}>
        <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-4 shadow-xl max-w-lg w-full mx-4">
          <h3 className="text-xl font-semibold text-zevyron-text mb-3">
            Welcome to The Apps Page!
          </h3>

          <div className="text-zevyron-text-secondary text-sm leading-6 mb-6">
            <p className="mb-4">
              Zevyron can install apps using either{" "}
              <strong className="text-zevyron-primary">Winget</strong> or{" "}
              <strong className="text-zevyron-primary">Chocolatey</strong>.{" "}
              <p className="text-zevyron-secondary">
            ...
            <div className="space-y-3">
              <button
                ...
                className={`w-full p-4 rounded-lg border transition-all text-left bg-zevyron-accent border-zevyron-text-muted hover:bg-zevyron-bg cursor-pointer hover:scale-[102%]`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-zevyron-text">Winget</h4>
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-zevyron-text">Chocolatey</h4>
                    <p className="text-sm text-zevyron-text-secondary">
            ...
            {(wingetChecking || chocolateyChecking) && (
              <p className="text-sm text-zevyron-text-secondary mt-4 text-center">
        ...
        <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-4 shadow-xl max-w-lg w-full mx-4">
          ...
          <div className="text-zevyron-text-secondary text-sm leading-6 whitespace-pre-wrap max-h-64 overflow-y-auto custom-scrollbar mb-6">
            {selectedApps.length > 0 ? (
                ...
                {selectedApps.map((appId) => {
                  ...
                  return (
                    <div className="flex  items-center gap-2" key={appId}>
                      <li key={appId} className="flex items-center gap-2 text-zevyron-text">
                        {app?.name || appId}
                      </li>
              ...
              <p className="text-zevyron-text-secondary italic">No apps selected</p>
            )}
          </div>
            ...
            {importedApps.length > 0 ? (
              <ul className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                {importedApps.map((id) => {
                  ...
                  return (
                    <li key={id} className="flex items-center gap-2 text-zevyron-text">
                      <Checkbox
              ...
              <p className="text-zevyron-text-secondary italic">No apps found in file</p>
            )}
          </div>
      ...
      <RootDiv>
        ...
        {wingetInstalled === false && (
          <Card className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 w-full mt-5 flex gap-4 items-center">
            ...
            <div className="flex-1">
              <h1 className="font-medium text-zevyron-text">Winget Not Installed</h1>
              <p className="text-zevyron-text-secondary">
        ...
        {source === "Chocolatey" && chocolateyInstalled === false && (
          <Card className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 w-full mt-5 flex gap-4 items-center">
            ...
            <div className="flex-1">
              <h1 className="font-medium text-zevyron-text">Chocolatey Not Installed</h1>
              <p className="text-zevyron-text-secondary">
        ...
        <div className="flex gap-3 mt-5 w-auto ml-1 mr-1">
          ...
          <Button
            className="text-zevyron-text flex gap-2"
          ...
          <label className="flex gap-2 cursor-pointer bg-zevyron-border text-zevyron-text rounded-lg font-medium px-3 py-1.5 text-sm text-center items-center active:scale-90 hover:bg-zevyron-secondary transition-all duration-200">
          ...
          {selectedApps.length > 0 && (
            <Button
              className="flex gap-2 ml-auto bg-zevyron-border text-zevyron-text"
              variant="secondary"
        ...
        <p className="mb-2 mt-2 text-zevyron-text-muted font-medium">
          Looking to debloat windows? its located in {""}
          <a className="text-zevyron-primary cursor-pointer" onClick={() => router("/tweaks")}>
            Tweaks
          </a>
        </p>
        ...
        <div className="flex flex-row gap-2 items-center">
          ...
          <div className="ml-auto flex gap-2 items-center">
            <p className="text-zevyron-text-muted">Select Source:</p>
            <Dropdown
        ...
        <div className="space-y-10 mb-10">
          {Object.entries(appsByCategory).map(([category, apps]) => (
            <div key={category} className="space-y-4">
              <h2 className="text-2xl text-zevyron-primary font-bold capitalize">{category}</h2>
                      ...
                      <Card onClick={() => toggleApp(appId)} className="p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            ...
                            {hideAppIcons !== true && (
                              <div className="min-w-10 max-w-10 max--h-10 min-h-10 rounded-lg overflow-hidden bg-zevyron-accent flex items-center justify-center">
                                {app.icon ? (
                                  <img
                                    src={app.icon}
                                    alt={app.name}
                            ...
                            <div>
                              <h3 className="text-zevyron-text font-medium group-hover:text-zevyron-primary transition">
                                {app.name}
                              </h3>
                              {app.info && (
                                <p className="text-sm text-zevyron-text-secondary line-clamp-1 font-semibold">
                                  {app.info}
                                </p>
                              )}
                              <p className="text-xs text-zevyron-text-secondary">ID: {appId}</p>
                            </div>
                          </div>
                          {app.link && (
                            <div onClick={(e) => e.stopPropagation()}>
                              <button
                                ...
                                className="ml-3 text-zevyron-primary hover:text-zevyron-text-secondary transition-opacity"
                                onClick={(e) => {
                                  e.stopPropagation()
                                  try {
          ...
          <p className="text-center text-zevyron-text-muted">Zevyron App Catalog</p>
        </div>
      </RootDiv>
    </>
  )
