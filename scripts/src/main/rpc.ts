...
const clientId = process.env.ZEVYRON_DISCORD_CLIENT_ID || ""
...
async function startDiscordRPC(): Promise<boolean> {
  if (!clientId) {
    log.warn("(rpc) Zevyron Discord Client ID is not configured")
    return false
  }
    ...
    try {
      ...
      rpcClient.on("READY", () => {
        ...
        const activity = new PresenceBuilder()
          ...
          .setState(`Running Zevyron v${jsonData.version ?? "2"}`)
          .setLargeImage("zevyron", "Zevyron")
