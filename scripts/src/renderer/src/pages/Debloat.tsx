    ...
    <>
      ...
      <RootDiv className="flex flex-col w-full h-full overflow-hidden">
        <Modal open={showConfirm} onClose={() => setShowConfirm(false)}>
          <div className="bg-zevyron-card border border-zevyron-border rounded-2xl p-6 shadow-xl max-w-md w-full mx-4">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2 rounded-full bg-red-500/20">
                <TriangleAlert className="w-6 h-6 text-red-500" />
              </div>
              <h3 className="text-xl font-semibold text-zevyron-text">Confirm Uninstall</h3>
            </div>
            ...
            <ul className="max-h-48 overflow-y-auto custom-scrollbar space-y-1 mb-6">
              {selectedNames.map((name) => (
                <li
                  key={name}
                  className="text-sm text-zevyron-text bg-zevyron-accent/50 rounded px-3 py-1.5"
                >
                  {name}
                </li>
              ))}
        ...
        <div className="flex items-center justify-end mb-4">
          <h1 className="text-2xl font-bold text-zevyron-text mr-auto">
            Installed Apps {!loading && !uninstalling && `(${apps.length})`}
            {uninstalling && (
              <span className="text-sm font-normal text-zevyron-text-secondary ml-2">
                Uninstalling...
              </span>
            )}
          </h1>
        ...
        <Table className="mb-2 h-[calc(100%-200px)] ">
          ...
          <TableBody>
            {loading ? (
              <tr className="border-b border-zevyron-border">
                <td colSpan={5} className="p-4 text-center text-zevyron-text-secondary">
              ...
              filteredApps.map((app) => (
                <tr
                  key={app.id}
                  className={`border-b border-zevyron-border transition-colors hover:bg-zevyron-accent/50 cursor-pointer ${selected.has(app.id) ? "bg-zevyron-accent/30" : ""}`}
                  onClick={() => !uninstalling && toggleOne(app.id)}
                >
                  <td className="p-4 align-middle text-zevyron-text w-10">
                    <div onClick={(e) => e.stopPropagation()}>
                      <Checkbox checked={selected.has(app.id)} onChange={() => toggleOne(app.id)} />
                    </div>
                  </td>
                  <td className="align-middle text-zevyron-text">
                    <div className="flex items-center gap-2">
                      {app.icon ? (
                        <img src={app.icon} alt={app.name} className="w-5 h-5" />
                      ) : (
                        <img src={zevyronLogo} alt={app.name} className="w-5 h-5" />
                      )}
                      <span>{app.name}</span>
                    </div>
                  </td>
                  <td className="p-4 align-middle text-zevyron-text">{app.publisher}</td>
                  <td className="p-4 align-middle text-zevyron-text">{app.version}</td>
                  <td className="p-4 align-middle text-zevyron-text">{app.installDate}</td>
                </tr>
              ))
            )}
          </TableBody>
